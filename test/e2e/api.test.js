...
describe('API E2E test Suite', () => {
  ...
  before(async () => {
    ...
    await new Promise((resolve, reject) => {
      _server.once('listening', () => {
        ...
        console.log('e2e is running on ', BASE_URL)
        resolve()
      })
    })
  })
  ...
  describe('/login', () => {
    ...
    it('should login successfully givin user and password', async () => {
      const input = {
        user: 'matthews',
        password: '654'
      }
      ...
      assert.ok(
        ...
        `response.body.token should be truthy, actual: ${JSON.stringify(response)}`
      )
  ...
  describe('/', () => {
    ...
    it('should be allowed to access private data with valid token', async () => {
      const input = { headers: { authorization: _globalToken } }
